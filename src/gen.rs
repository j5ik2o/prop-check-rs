...
impl Gens {
  ...
  where
    ...
    Self::choose_u32(1, total).flat_map(move |n| tree.range(n..).next().unwrap().1.clone())
  }
  ...
  pub fn one_of<T: Choose + Clone + 'static>(values: impl IntoIterator<Item = Gen<T>>) -> Gen<T> {
    ...
    Self::choose(0usize, vec.len() - 1).flat_map(move |idx| vec[idx].clone())
  }
  ...
  pub fn choose_char(min: char, max: char) -> Gen<char> {
    let chars = (min..=max).map(Self::pure).collect::<Vec<_>>();
    Self::one_of(chars)
  }
