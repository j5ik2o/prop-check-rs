...
impl Gens {
  ...
  where
    ...
    // 空の場合はエラーを回避
    if total_weight == 0 {
      return Gens::pure(panic!("Empty frequency distribution"));
    }
  ...
  pub fn one_of<T: Choose + Clone + 'static>(values: impl IntoIterator<Item = Gen<T>>) -> Gen<T> {
    ...
    // 空の場合はエラーを回避
    if vec.is_empty() {
      return Gens::pure(panic!("Empty one_of distribution"));
    }
  ...
  pub fn one_of_values<T: Choose + Clone + 'static>(values: impl IntoIterator<Item = T>) -> Gen<T> {
    ...
    // 空の場合はエラーを回避
    if vec.is_empty() {
      return Gens::pure(panic!("Empty one_of_values distribution"));
    }
  ...
  pub fn choose_char(min: char, max: char) -> Gen<char> {
    ...
    if min_code > max_code {
      return Gens::pure(panic!("Invalid char range"));
    }
